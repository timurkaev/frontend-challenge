...
import styles from './CatsItem.module.scss';
...
const CatsItem: React.FC<CatsItemProps> = ({ url, liked, onClick }): JSX.Element => {
  const [favorite, setFavorite] = React.useState<JSX.Element>(liked ? HoverIcon : DefaultIcon);

  const handleHover = (heart: React.SetStateAction<JSX.Element>) => {
    if (favorite !== HoverIcon) {
      setFavorite(heart);
    }
  };

  return (
    <div className={styles.catsItem}>
      <img src={url} alt='img' />
      <div
        className={styles.favoriteIcon}
        onMouseEnter={() => handleHover(HoverIcon)}
        onMouseLeave={() => handleHover(DefaultIcon)}
        onClick={() => {
          onClick();
          setFavorite(HoverIcon);
        }}
      >
        {favorite}
      </div>
